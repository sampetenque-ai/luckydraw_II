...
const LuckyDraw: React.FC<LuckyDrawProps> = ({ employees, onAddWinner, winners, onClearWinners }) => {
  ...
  const downloadWinnersCsv = () => {
    ...
    let csvContent = "得獎順序,姓名\n";
    winners.forEach((winner, index) => {
      csvContent += `${winners.length - index},${winner.name}\n`;
    });
    ...
    link.setAttribute("download", `得獎名單_${new Date().toLocaleDateString()}.csv`);
    ...
    <div className="flex flex-col items-center py-12 px-4 max-w-5xl mx-auto relative">
      ...
      {showClearConfirm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl p-6 max-w-sm w-full transform animate-in zoom-in-95 duration-200">
            <h3 className="text-xl font-bold text-slate-800 mb-2">確定要清空紀錄嗎？</h3>
            <p className="text-slate-500 mb-6">此動作無法復原，所有得獎紀錄將會消失。</p>
            <div className="flex space-x-3">
              ...
              <button
                ...
                className="flex-1 py-2.5 bg-red-500 text-white rounded-xl font-bold hover:bg-red-600 transition-colors shadow-red-200 shadow-lg"
              >
                確定清空
              </button>
            </div>
      ...
      <div className="w-full bg-white rounded-[3rem] shadow-2xl overflow-hidden border border-slate-100 flex flex-col items-center p-12 relative">
        ...
        <div className="flex flex-col sm:flex-row gap-10 items-center w-full justify-center">
          ...
          <button
            ...
            className={`px-20 py-5 rounded-[1.5rem] font-black text-2xl shadow-2xl transform transition-all active:scale-90 ${isSpinning || availableEmployees.length === 0
          ...
          {winners.length > 0 && (
            <div className="flex items-center space-x-4">
              ...
              <button
                ...
                className="text-sm font-bold text-red-400 hover:text-red-600 transition-colors uppercase tracking-widest"
              >
                清空紀錄
              </button>
            </div>
